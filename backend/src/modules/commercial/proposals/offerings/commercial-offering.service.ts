  ...
  public static async listCategories(): Promise<CommercialOfferingCategoryDTO[]> {
    ...
    return categories.map((cat: any) => ({
      ...
      offerings: offerings
        ...
        .map((off: any) => ({
          id: off.id,
          publicCode: off.publicCode || `OFR-2026-${String(off.sortOrder || 1).padStart(6, '0')}`,
          categoryId: off.categoryId,
          categoryName: cat.name,
          code: off.code,
          name: off.name,
          type: off.type || 'SERVICE',
          description: off.description,
          status: off.status || 'ACTIVE',
          currentVersionId: off.currentVersionId || null,
          currentVersionNumber: off.currentVersionNumber || 1,
