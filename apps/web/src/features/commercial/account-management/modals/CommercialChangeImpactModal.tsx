      ...
      <div className="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-800/50 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-white">Simulador de Impacto Comercial</h2>
        ...
        {/* Configuration Bar */}
        <div className="p-4 bg-slate-800/30 border-b border-slate-800 flex flex-wrap items-center gap-4 shrink-0">
          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold text-slate-300">Movimentação:</span>
            <select
              value={movementType}
          ...
          <div className="flex items-center gap-2 flex-1 min-w-[200px]">
            <span className="text-xs font-semibold text-slate-300">Oferta Pretendida:</span>
            <select
          ...
          <button
            ...
            disabled={isLoading}
            className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-medium rounded-lg transition flex items-center gap-1.5 disabled:opacity-50"
          >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {impactData && (
            <>
              ...
              <div className="p-3 bg-indigo-500/10 border border-indigo-500/30 rounded-lg flex items-center justify-between text-indigo-300 text-xs">
                <div className="flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="p-4 bg-slate-800/40 border border-slate-700/60 rounded-xl space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="px-2 py-0.5 text-xs bg-emerald-500/10 text-emerald-400 rounded-full font-medium">
                    ...
                    {impactData.impactAnalysis.featuresAdded.length === 0 ? (
                      ...
                      <div className="space-y-1.5">
                        {impactData.impactAnalysis.featuresAdded.map((f) => (
                          <div key={f.key} className="p-2 bg-emerald-500/5 border border-emerald-500/20 rounded-lg text-xs flex justify-between items-center">
                            <span className="font-medium text-emerald-300">{f.name}</span>
                            <span className="text-[10px] text-slate-400 uppercase">{f.category}</span>
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  <div className="p-4 bg-slate-800/40 border border-slate-700/60 rounded-xl space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="px-2 py-0.5 text-xs bg-rose-500/10 text-rose-400 rounded-full font-medium">
                    ...
                    {impactData.impactAnalysis.featuresRemoved.length === 0 ? (
                      ...
                      <div className="space-y-1.5">
                        {impactData.impactAnalysis.featuresRemoved.map((f) => (
                          <div key={f.key} className="p-2 bg-rose-500/10 border border-rose-500/30 rounded-lg text-xs flex justify-between items-center">
                            <span className="font-medium text-rose-300">{f.name}</span>
                            <span className="text-[10px] text-slate-400 uppercase">{f.category}</span>
                          </div>
                        ))}
                      </div>
                      ...
                      <tbody className="divide-y divide-slate-800 text-slate-200">
                        {impactData.impactAnalysis.limitChanges.map((lc, idx) => (
                          <tr key={idx} className="hover:bg-slate-800/20">
                            <td className="px-4 py-2 font-medium text-white">{lc.featureName}</td>
                            <td className="px-4 py-2 text-slate-400">{lc.limitKey}</td>
                            <td className="px-4 py-2 text-slate-300">{lc.currentValue !== null ? lc.currentValue : 'Ilimitado'}</td>
                            <td className="px-4 py-2 font-semibold text-white">{lc.proposedValue !== null ? lc.proposedValue : 'Ilimitado'}</td>
                            <td className="px-4 py-2">
                              {lc.difference !== null ? (
                                <span className={`font-semibold ${lc.difference > 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-4 bg-slate-800/30 border border-slate-700 rounded-xl space-y-2">
                    <span className="text-xs font-semibold text-slate-400">Condições Vigentes (Base Efetiva)</span>
                    <div className="space-y-1 text-xs">
                      {impactData.impactAnalysis.termComparison.currentTerms.length === 0 ? (
                        ...
                        impactData.impactAnalysis.termComparison.currentTerms.map((t, idx) => (
                          <div key={idx} className="flex justify-between py-1 border-b border-slate-800 text-slate-300">
                            <span>{t.type}</span>
                  ...
                  <div className="p-4 bg-indigo-500/5 border border-indigo-500/20 rounded-xl space-y-2">
                    <span className="text-xs font-semibold text-indigo-400">Condições Pretendidas ({impactData.proposedOffering.name})</span>
                    <div className="space-y-1 text-xs">
                      {impactData.impactAnalysis.termComparison.proposedTerms.map((t, idx) => (
                        <div key={idx} className="flex justify-between py-1 border-b border-slate-800 text-indigo-200">
                          <span>{t.type}</span>
