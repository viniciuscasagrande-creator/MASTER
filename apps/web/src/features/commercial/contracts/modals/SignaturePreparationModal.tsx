      ...
      <div className="relative w-full max-w-2xl overflow-hidden rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 bg-slate-950/60 px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-base font-bold text-white">Preparar Envelope para Assinatura Digital</h2>
              <p className="text-xs text-slate-400">
                Despacho formal para o contrato <span className="font-mono text-white">{contractCode}</span>
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto p-6 space-y-5 text-sm flex-1">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="flex items-center gap-1 rounded bg-slate-800 px-2.5 py-1 text-xs font-medium text-purple-400 hover:bg-slate-700 hover:text-purple-300 border border-slate-700"
              >
            ...
            <div className="space-y-3">
              {signers.map((signer, index) => (
                <div
                  key={index}
                  className="rounded-xl border border-slate-800 bg-slate-950/40 p-3.5 space-y-2.5 relative"
                >
                  <div className="flex items-center justify-between">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[10px] text-slate-400 mb-0.5">Nome Completo *</label>
                      <input
                        type="text"
                        value={signer.name}
                        onChange={e => handleUpdateSigner(index, 'name', e.target.value)}
                        placeholder="Nome do signatário"
                        className="w-full rounded-md border border-slate-700 bg-slate-800 px-2.5 py-1.5 text-xs text-white focus:outline-none"
                        required
                      />
                    </div>

                    <div>
                      <label className="block text-[10px] text-slate-400 mb-0.5">E-mail para Assinatura *</label>
                      <input
                        type="email"
                        value={signer.email}
                    ...
                    <div>
                      <label className="block text-[10px] text-slate-400 mb-0.5">CPF / Documento</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-[10px] text-slate-400 mb-0.5">Cargo / Papel Jurídico</label>
                      <input
                        type="text"
                        value={signer.role}
          ...
          <div className="rounded-lg border border-slate-800 bg-slate-950/30 p-3 text-xs text-slate-400 flex items-start gap-2">
