  ...
  return (
    ...
    >
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-5xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden text-slate-100">
        {/* Header */}
        <div className="bg-slate-950 px-6 py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                Canal: <strong className="text-slate-200">{order.salesChannelName || 'Site Oficial'}</strong> • Criado em: {formatDateTime(order.createdAt)}
              </p>
            </div>
          </div>
        ...
        {/* Action Alerts */}
        {actionSuccess && (
          <div className="px-6 py-2.5 bg-emerald-500/10 border-b border-emerald-500/30 text-emerald-400 text-xs flex items-center gap-2">
            <CheckCircle2 className="w-4 h-4 shrink-0" />
            <span>{actionSuccess}</span>
          </div>
        )}
        {actionError && (
          <div className="px-6 py-2.5 bg-rose-500/10 border-b border-rose-500/30 text-rose-400 text-xs flex items-center gap-2">
        ...
        {/* Tab Navigation */}
        <div className="flex items-center gap-1 px-6 bg-slate-950/60 border-b border-slate-800 overflow-x-auto select-none">
          {[
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-4 space-y-3">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-white">{order.buyerSnapshot?.name || 'Cliente Balcão'}</h4>
                    <p className="text-xs text-slate-400 font-mono mt-0.5">{order.buyerSnapshot?.documentMasked || 'Não informado'}</p>
                    <p className="text-xs text-slate-400 font-mono">{order.buyerSnapshot?.emailMasked || 'Não informado'}</p>
                    {order.buyerSnapshot?.phoneMasked && (
                      <p className="text-xs text-slate-400 font-mono">{order.buyerSnapshot.phoneMasked}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-4 space-y-3">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-white">{order.eventName || 'Evento Geral'}</h4>
                ...
                <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-4 space-y-2">
                  ...
                  <div className="space-y-1.5 text-xs">
                    <div className="flex justify-between text-slate-400">
                      ...
                      <span className="font-mono text-slate-200">{formatCurrency(order.subtotalAmount)}</span>
                    </div>
                    <div className="flex justify-between text-slate-400">
                      <span>Taxa de Conveniência:</span>
                      <span className="font-mono text-slate-200">{formatCurrency(order.feeAmount)}</span>
                    </div>
                    ...
                    <div className="flex justify-between text-white font-bold pt-2 border-t border-slate-800 text-sm">
                      ...
                      <span className="font-mono text-orange-400">{formatCurrency(order.totalAmount)}</span>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  {order.items && order.items.length > 0 ? (
                    order.items.map((item, idx) => (
                      <div key={item.id || idx} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                        <div>
                          <div className="flex items-center gap-2">
                            <strong className="text-white font-semibold">{item.ticketTypeName || 'Ingresso Geral'}</strong>
                            {item.sectionName && (
                          ...
                          <p className="text-slate-400 text-[11px] mt-0.5">
                            Qtd: <strong className="text-slate-200">{item.quantity}</strong> • Valor unitário: {formatCurrency(item.unitFinalAmount || item.unitBaseAmount)}
                          </p>
                        </div>
          ...
          {activeTab === 'ingressos' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-bold text-white">Credenciais de Acesso & Ingressos</h3>
              ...
              <div className="space-y-3">
                {order.items && order.items.length > 0 ? (
                  order.items.map((item, idx) => (
                    <div
                      key={item.id || idx}
                      className="bg-slate-950/60 border border-slate-800 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4"
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <h4 className="text-xs font-bold text-white">{item.ticketTypeName || 'Ingresso Oficial'}</h4>
                          ...
                          <p className="text-[11px] text-slate-400 mt-0.5">
                            Titular: <strong className="text-slate-300">{order.buyerSnapshot?.name || 'Titular da Compra'}</strong>
                          </p>
                        </div>
                      </div>
              ...
              <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-5 space-y-4">
                <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <h3 className="text-sm font-bold text-white">Dados do Gateway de Pagamento</h3>
                  </div>
                  {getStatusBadge(order.status)}
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 text-xs">
                  <div>
                    ...
                    <strong className="text-white">Cartão de Crédito / PIX</strong>
                  </div>
                  <div>
                    <span className="text-slate-400 block text-[11px]">Moeda:</span>
                    <strong className="text-white font-mono">{order.currency || 'BRL'}</strong>
                  </div>
                  <div>
                    <span className="text-slate-400 block text-[11px]">Canal de Venda:</span>
                    <strong className="text-white">{order.salesChannelName || 'Site Oficial'}</strong>
                  </div>
                  <div>
                    <span className="text-slate-400 block text-[11px]">Status de Liquidação:</span>
                    <strong className="text-emerald-400">Conciliado no Ledger</strong>
                  </div>
                </div>
              </div>
          ...
          {activeTab === 'timeline' && (
            <div className="space-y-4">
              <h3 className="text-sm font-bold text-white">Histórico de Eventos da Ordem</h3>
                    ...
                    <div key={ev.id || idx} className="relative">
                      ...
                      <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-3 text-xs">
                        <div className="flex items-center justify-between text-slate-400 text-[11px]">
                          <span className="font-bold text-orange-400 uppercase tracking-wider">{ev.eventType}</span>
                          <span>{formatDateTime(ev.createdAt)}</span>
                        </div>
                        <p className="text-slate-200 mt-1">{ev.description}</p>
                        {ev.actorName && (
                          <p className="text-slate-400 text-[10px] mt-0.5">Ator: {ev.actorName} ({ev.actorType})</p>
                        )}
                      </div>
                    </div>
                  ))}
          ...
          {activeTab === 'developer' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-bold text-white">Snapshot de Integridade do Core</h3>
                <button
