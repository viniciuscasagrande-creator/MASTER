    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold font-mono text-slate-900 dark:text-white tracking-tight">
                {order.publicCode}
              </h1>
              <Badge
                variant={
              ...
              <span className="rounded bg-slate-100 dark:bg-slate-800 px-2 py-0.5 text-[10px] font-mono text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                v{order.version || 1}
              </span>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0F172A] p-5 shadow-xs space-y-3">
          ...
          <div className="space-y-2 text-xs divide-y divide-slate-100 dark:divide-slate-800/80">
            ...
            <div className="flex justify-between text-slate-900 dark:text-white font-bold pt-3 text-sm">
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0F172A] p-5 shadow-xs space-y-3 md:col-span-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-slate-900 dark:text-white font-bold text-sm">
              <User className="h-4 w-4 text-cyan-600 dark:text-cyan-400" />
              <span>Dados do Comprador</span>
            </div>
            <span className="inline-flex items-center gap-1 text-[10px] text-cyan-600 dark:text-cyan-400 font-medium">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs pt-1">
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/40 p-3 space-y-1">
              <div className="text-[11px] text-slate-500 dark:text-slate-400">Nome do Titular</div>
              <div className="font-medium text-slate-900 dark:text-white text-sm">{buyer?.name || 'Não informado'}</div>
            </div>

            <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/40 p-3 space-y-1">
              <div className="text-[11px] text-slate-500 dark:text-slate-400">CPF / Documento</div>
            ...
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/40 p-3 space-y-1">
              <div className="text-[11px] text-slate-500 dark:text-slate-400">E-mail Cadastrado</div>
              <div className="font-mono text-slate-900 dark:text-white text-sm">
            ...
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/40 p-3 space-y-1">
              <div className="text-[11px] text-slate-500 dark:text-slate-400">Telefone / WhatsApp</div>
              <div className="font-mono text-slate-900 dark:text-white text-sm">
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0F172A] p-5 shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5 text-[11px] text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/80 px-2.5 py-1 rounded-lg border border-slate-200 dark:border-slate-700">
              ...
              {order.items?.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                  <td className="py-3 px-3">
                    <div className="font-semibold text-slate-900 dark:text-white">{item.ticketTypeName}</div>
                    <div className="text-[10px] text-slate-500 font-mono">ID: {item.eventTicketTypeId}</div>
                  </td>
                  <td className="py-3 px-3">
                    <div className="text-slate-700 dark:text-slate-200">{item.sectionName || 'Setor Padrão'}</div>
                    <div className="text-[10px] text-slate-500 dark:text-slate-400">{item.sessionName || 'Sessão Principal'}</div>
                  </td>
        ...
        <div className="space-y-3 relative border-l-2 border-slate-200 dark:border-slate-800 ml-3 pl-4">
          {order.timeline?.map((ev, idx) => (
            <div key={idx} className="relative space-y-1">
              {/* Dot */}
              <div className="absolute -left-[23px] top-1 h-3 w-3 rounded-full border-2 border-white dark:border-slate-900 bg-orange-500" />

              <div className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  <span className="font-bold text-slate-900 dark:text-white font-mono">{ev.eventType}</span>
                  <Badge variant="slate" size="sm">
                    {ev.actorType}
                  </Badge>
                  {ev.actorName && (
                    <span className="text-slate-500 dark:text-slate-400">• {ev.actorName}</span>
                  )}
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 dark:bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-2xl border border-rose-200 dark:border-rose-500/30 bg-white dark:bg-slate-900 p-6 shadow-2xl space-y-4">
            ...
            <p className="text-xs text-slate-600 dark:text-slate-300">
              Você está prestes a cancelar o pedido <strong className="font-mono text-slate-900 dark:text-white">{order.publicCode}</strong>. Esta ação é irreversível na esteira comercial e registrará evento permanente na linha do tempo auditada.
            </p>
            ...
            <div className="space-y-1">
              ...
              <textarea
                ...
                className="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 p-2.5 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:border-rose-500 focus:outline-none focus:bg-white"
              />
            </div>
