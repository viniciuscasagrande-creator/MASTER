  ...
  if (error || !offering) {
    return (
      <div className="p-8 text-center bg-slate-900 border border-slate-800 rounded-xl space-y-4">
        <p className="text-rose-400 font-semibold">{error || 'Oferta comercial não encontrada.'}</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-6 bg-slate-900 border border-slate-800 rounded-xl shadow-lg space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1.5">
            <div className="flex items-center gap-3">
              <span className="font-mono text-xs font-bold text-indigo-400 bg-indigo-950/60 px-2.5 py-0.5 rounded border border-indigo-800/40">
                {offering.publicCode}
              </span>
              <span className="text-xs text-slate-400 font-mono">({offering.code})</span>
              <span className="text-xs px-2 py-0.5 rounded-full font-semibold bg-indigo-500/10 text-indigo-300 border border-indigo-500/20">
                {offering.type}
              </span>
              <span
            ...
            <h1 className="text-2xl font-bold text-white">{offering.name}</h1>
            <p className="text-xs text-slate-400">
              Categoria: <strong className="text-slate-200">{offering.categoryName || 'Geral'}</strong> | Versão Ativa Corrente:{' '}
              <strong className="text-indigo-400 font-mono">v{offering.currentVersionNumber}</strong>
            </p>
          </div>

          <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl min-w-[220px] text-right space-y-1">
        ...
        {currentVer?.contentHash && (
          <div className="p-3 bg-slate-950/80 border border-slate-800 rounded-lg flex items-center justify-between text-xs">
            <div className="flex items-center gap-2 text-slate-400">
              ...
              <span className="font-mono text-emerald-400 font-medium">{currentVer.contentHash}</span>
            </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-lg">
        <div className="flex border-b border-slate-800 px-6 pt-4 gap-6 text-sm overflow-x-auto">
          {[
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-4 bg-slate-950 rounded-xl border border-slate-800 space-y-2">
                  <h4 className="text-xs font-semibold text-slate-400">Atributos Operacionais</h4>
                  <div className="space-y-1.5 text-xs">
                    <div className="flex justify-between py-1 border-b border-slate-900">
                      <span className="text-slate-500">Código Oficial:</span>
                      <span className="font-mono text-slate-200">{offering.code}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-slate-900">
                      <span className="text-slate-500">Código Público:</span>
                      <span className="font-mono text-indigo-400">{offering.publicCode}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-slate-900">
                      <span className="text-slate-500">Tipo de Estrutura:</span>
                      <span className="text-slate-200">{offering.type}</span>
                    </div>
                    <div className="flex justify-between py-1">
                      <span className="text-slate-500">Ordem de Exibição:</span>
                      <span className="text-slate-200">{offering.sortOrder}</span>
                    </div>
                  </div>
                </div>

                <div className="p-4 bg-slate-950 rounded-xl border border-slate-800 space-y-2">
                  <h4 className="text-xs font-semibold text-slate-400">Metadados de Versionamento</h4>
                  <div className="space-y-1.5 text-xs">
                    <div className="flex justify-between py-1 border-b border-slate-900">
                      <span className="text-slate-500">Versão Vigente:</span>
                      <span className="font-mono text-emerald-400">v{offering.currentVersionNumber}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-slate-900">
                      <span className="text-slate-500">Total de Versões:</span>
                      <span className="text-slate-200">{versions.length} registrada(s)</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-slate-900">
                      <span className="text-slate-500">Cadastrado em:</span>
                      <span className="text-slate-200">{formatDate(offering.createdAt)}</span>
                    </div>
                    <div className="flex justify-between py-1">
                      <span className="text-slate-500">Última Atualização:</span>
                      <span className="text-slate-200">{formatDate(offering.updatedAt)}</span>
                    </div>
                  </div>
                </div>
              </div>
                    ...
                    >
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          {comp.required && (
                            <span className="text-[10px] text-amber-400 border border-amber-500/20 bg-amber-500/10 px-1.5 py-0.5 rounded">
                              Obrigatório
                            </span>
                          )}
                        </div>
                      ...
                      <div className="flex items-center gap-6">
                        <div className="text-right">
                          ...
                          <p className="text-sm font-bold text-slate-200">{comp.quantity}x</p>
                        </div>
                  ...
                  {currentVer.features.map(f => (
                    ...
                    >
                      ...
                      <div className="flex-1 space-y-1">
                        <div className="flex items-center justify-between">
                          <p className="text-xs font-semibold text-white">{f.featureName || f.featureCode}</p>
                          <span className="text-[10px] font-mono text-slate-500 bg-slate-900 px-1.5 py-0.5 rounded">
                            {f.featureCategory}
                          </span>
                        </div>
                        <p className="text-[11px] font-mono text-indigo-400">{f.featureCode}</p>
                ...
                <div className="divide-y divide-slate-800 border border-slate-800 rounded-xl overflow-hidden bg-slate-950">
                  {currentVer.defaultTerms.map(term => (
                    <div key={term.id} className="p-4 flex items-center justify-between">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-semibold text-white">{term.termType}</span>
                          <span className="text-[10px] px-2 py-0.5 rounded bg-slate-900 text-slate-400 border border-slate-800">
                            {term.calculationType}
                          </span>
                        </div>
                        <p className="text-xs text-slate-400">
                          Responsável pelo Pagamento: <strong className="text-slate-200">{term.payer}</strong>
                        </p>
                      </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl text-center">
                      <p className="text-xs text-slate-400">Propostas em Rascunho</p>
                      <p className="text-2xl font-bold text-amber-400 pt-1">{impact.draftProposalsCount}</p>
                    </div>
                    <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl text-center">
                      <p className="text-xs text-slate-400">Propostas Enviadas</p>
                      <p className="text-2xl font-bold text-indigo-400 pt-1">{impact.sentProposalsCount}</p>
                    </div>
                    <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl text-center">
                      <p className="text-xs text-slate-400">Contratos Ativos</p>
                      <p className="text-2xl font-bold text-emerald-400 pt-1">{impact.activeContractsCount}</p>
                    </div>
                    <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl text-center">
                      <p className="text-xs text-slate-400">Pacotes Pai</p>
                      <p className="text-2xl font-bold text-purple-400 pt-1">{impact.parentPackagesCount}</p>
                    </div>
                  </div>

                  <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl space-y-2 text-xs">
                    <h4 className="font-semibold text-slate-200">Diagnóstico de Dependências:</h4>
                    <p className="text-slate-300">{impact.warningMessage}</p>
                  </div>
                </div>
              ) : (
                <p className="text-xs text-slate-500 py-6 text-center">Nenhum dado de impacto disponível.</p>
              )}
            </div>
          )}
        </div>
