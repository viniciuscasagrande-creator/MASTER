...
export const NewVersionModal: React.FC<NewVersionModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="bg-slate-900 border border-slate-800 rounded-xl shadow-2xl w-full max-w-2xl flex flex-col text-slate-100 animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between p-6 border-b border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white">Criar Nova Versão (v{nextVersionNumber} Rascunho)</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1.5">
              Justificativa / Resumo das Alterações (Obrigatório) <span className="text-rose-400">*</span>
            </label>
            <input
              type="text"
              value={changeSummary}
          ...
          <div className="p-4 bg-slate-950/60 border border-slate-800/80 rounded-xl space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div>
                <label className="block text-xs text-slate-400 mb-1">Valor Fixo (R$)</label>
                <input
                  type="number"
                  step="0.01"
                  value={amount}
              ...
              <div>
                <label className="block text-xs text-slate-400 mb-1">Garantia Mínima (R$)</label>
                <input
                  type="number"
                  step="0.01"
                  value={minimumAmount}
