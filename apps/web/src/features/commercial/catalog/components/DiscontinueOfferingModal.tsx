...
export const DiscontinueOfferingModal: React.FC<DiscontinueOfferingModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 dark:bg-black/75 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl w-full max-w-xl text-slate-900 dark:text-slate-100 p-6 space-y-5 animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-xl text-amber-600 dark:text-amber-400">
              <AlertTriangle className="h-5 w-5" />
            </div>
            <div>
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">Descontinuar Oferta Comercial</h2>
        ...
        {error && (
          <div className="p-3 bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 text-rose-700 dark:text-rose-400 rounded-xl text-sm">
            {error}
          </div>
        )}
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              <div className="p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-center">
                ...
                <p className="text-lg font-bold text-amber-600 dark:text-amber-400">{impact.draftProposalsCount}</p>
              </div>
              <div className="p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-center">
                <p className="text-[10px] text-slate-500 uppercase font-semibold">Enviadas</p>
                <p className="text-lg font-bold text-indigo-600 dark:text-indigo-400">{impact.sentProposalsCount}</p>
              </div>
              <div className="p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-center">
                <p className="text-[10px] text-slate-500 uppercase font-semibold">Contratos Ativos</p>
                <p className="text-lg font-bold text-emerald-600 dark:text-emerald-400">{impact.activeContractsCount}</p>
              </div>
              <div className="p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-center">
                <p className="text-[10px] text-slate-500 uppercase font-semibold">Pacotes Pai</p>
                <p className="text-lg font-bold text-purple-600 dark:text-purple-400">{impact.parentPackagesCount}</p>
              </div>
            </div>

            {impact.parentPackages.length > 0 && (
              <div className="p-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs space-y-1">
                <span className="text-slate-500 dark:text-slate-400 font-semibold">Pacotes que utilizam este item:</span>
            ...
            <div className="p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-600 dark:text-slate-300 flex items-start gap-2">
              <ShieldAlert className="h-4 w-4 text-emerald-600 dark:text-emerald-400 shrink-0 mt-0.5" />
              <span>
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">
            Motivo da Descontinuação <span className="text-rose-500">*</span>
          </label>
          <textarea
            rows={2}
            value={reason}
            ...
            className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-sm text-slate-900 dark:text-slate-200 placeholder:text-slate-400 focus:outline-none focus:border-amber-500 focus:bg-white"
            required
          />
        </div>
