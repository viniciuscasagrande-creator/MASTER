      ...
      <div className="relative w-full max-w-4xl bg-slate-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-900/80">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-white">Comparativo Estruturado de Versões</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-3 bg-slate-950 border-b border-slate-800 flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-3">
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Versão Base:</span>
            <select
              value={base}
              onChange={(e) => setBase(Number(e.target.value))}
              className="bg-slate-800 text-white text-sm border border-slate-700 rounded px-2.5 py-1 focus:outline-none focus:border-indigo-500"
            >
            ...
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Versão Comparada:</span>
            <select
              value={target}
          ...
          {diff && (
            <div className="flex items-center gap-3 text-xs text-slate-400">
              <span className="flex items-center gap-1">
                ...
                V{base}: <code className="text-slate-300 font-mono">{diff.contentHashBase.substring(0, 8)}...</code>
              </span>
              <span className="flex items-center gap-1">
                <Hash className="w-3.5 h-3.5 text-emerald-400" />
                V{target}: <code className="text-slate-300 font-mono">{diff.contentHashTarget.substring(0, 8)}...</code>
              </span>
            </div>
          )}
        </div>
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-4">
          {loading ? (
            ...
            <div className="p-4 bg-rose-900/30 border border-rose-800 rounded-lg text-rose-300 text-sm flex items-center gap-2">
            ...
            <div className="py-12 text-center text-slate-400">
              ...
              <h3 className="text-base font-semibold text-white">Nenhuma diferença detectada</h3>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-semibold px-2 py-0.5 rounded border uppercase text-slate-300 bg-slate-800">
                    ...
                    {item.changeType === 'MODIFIED' && item.oldValue && item.newValue && typeof item.oldValue === 'object' && (
                      <div className="grid grid-cols-2 gap-3 mt-3 pt-3 border-t border-slate-900 text-xs font-mono">
                        <div className="p-2 rounded bg-slate-900/80 border border-slate-800 text-slate-400">
                          <span className="text-[10px] uppercase font-bold text-rose-400 block mb-1">Versão V{base}</span>
                        ...
                        <div className="p-2 rounded bg-slate-900/80 border border-slate-800 text-slate-300">
                          <span className="text-[10px] uppercase font-bold text-emerald-400 block mb-1">Versão V{target}</span>
