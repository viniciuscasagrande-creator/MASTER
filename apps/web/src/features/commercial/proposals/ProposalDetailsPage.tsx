...
export const ProposalDetailsPage: React.FC<ProposalDetailsPageProps> = ({
  ...
  return (
    <div className="space-y-6 pb-16">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            icon={<RefreshCw className={`h-3.5 w-3.5 mr-1 ${loading ? 'animate-spin' : ''}`} />}
          >
            Recarregar
          </Button>
        </div>
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/80 p-6 shadow-xs relative overflow-hidden backdrop-blur-sm">
        ...
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6 relative z-10">
          ...
          <div className="space-y-3">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="rounded bg-slate-100 dark:bg-slate-800 px-2.5 py-1 text-xs font-mono font-semibold text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 flex items-center gap-1">
            ...
            <h1 className="text-xl font-bold text-slate-900 dark:text-white">{proposal.title}</h1>
            ...
            <div className="flex flex-wrap items-center gap-y-2 gap-x-6 text-xs text-slate-500 dark:text-slate-400">
              <div className="flex items-center gap-1.5">
                ...
                <span>Produtor: </span>
                <strong
                  className="text-slate-800 dark:text-slate-200 hover:text-orange-600 dark:hover:text-orange-400 transition-colors cursor-pointer"
                ...
                {(proposal as any).producerDocument && (
                  <span className="text-slate-400 dark:text-slate-500">({(proposal as any).producerDocument})</span>
                )}
              </div>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span>Validade: </span>
                <strong className={isExpired() ? 'text-rose-600 dark:text-rose-400' : isExpiringSoon() ? 'text-amber-600 dark:text-amber-400' : 'text-slate-700 dark:text-slate-200'}>
              ...
              {currentVer?.contentHash && (
                <div className="flex items-center gap-1.5 bg-slate-50 dark:bg-slate-950/80 px-2.5 py-1 rounded border border-slate-200 dark:border-slate-800 text-[11px] font-mono text-slate-600 dark:text-slate-400">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {proposal.status === 'DRAFT' && (
              <>
                ...
                <Button
                  ...
                  className="bg-cyan-600 hover:bg-cyan-500 text-white shadow-xs"
                >
            ...
            {proposal.status === 'SENT' && (
              <>
                <Button
                  ...
                  className="bg-emerald-600 hover:bg-emerald-500 text-white shadow-xs font-semibold"
                >
            ...
            {proposal.status !== 'ACCEPTED' && proposal.status !== 'CANCELLED' && (
              <Button
                ...
                className="text-rose-600 hover:text-rose-700 hover:bg-rose-50 dark:text-rose-400 dark:hover:text-rose-300 dark:hover:bg-rose-500/10 h-8 px-2"
        ...
        {proposal.status === 'ACCEPTED' && (
          <div className="mt-5 p-4 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-500/40 text-emerald-800 dark:text-emerald-200 text-xs flex items-start gap-3">
            <CheckCircle2 className="h-5 w-5 text-emerald-600 dark:text-emerald-400 shrink-0 mt-0.5" />
            <div className="space-y-1">
              <strong className="text-emerald-900 dark:text-emerald-300 block text-sm">
                Proposta Aceita Formalmente pelo Produtor!
              </strong>
              <p className="text-slate-600 dark:text-slate-300">
                Condições comerciais formalizadas e aprovadas. Esta proposta está pronta para a celebração do contrato jurídico formal.
              </p>
              <p className="text-emerald-700 dark:text-emerald-400 font-semibold pt-1 border-t border-emerald-200 dark:border-emerald-900/60 text-[11px]">
        ...
        {proposal.status === 'APPROVAL_PENDING' && (
          <div className="mt-5 p-3.5 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-500/40 text-amber-800 dark:text-amber-200 text-xs flex items-center gap-3">
            <Clock className="h-5 w-5 text-amber-600 dark:text-amber-400 shrink-0" />
            <div>
              <strong className="text-amber-900 dark:text-amber-300">Aguardando Decisão da Alçada Comercial:</strong> Versão v{activeVerNum} submetida à aprovação da diretoria com restrição de Maker-Checker.
            </div>
          </div>
        )}

        {isExpired() && proposal.status !== 'ACCEPTED' && (
          <div className="mt-5 p-3.5 rounded-xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-500/40 text-rose-800 dark:text-rose-200 text-xs flex items-center justify-between">
            <div className="flex items-center gap-2">
      ...
      {activeTab === 'overview' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-5 space-y-4 shadow-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-3 border-t border-slate-100 dark:border-slate-800 text-xs">
                <div>
                  <span className="text-slate-500 dark:text-slate-400 block">Modelo Comercial Principal:</span>
                  <strong className="text-slate-900 dark:text-slate-200 text-sm">{formatModel(currentVer?.commercialModel || 'STANDARD')}</strong>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400 block">Tipo de Negociação:</span>
                  <strong className="text-slate-900 dark:text-slate-200 text-sm">B2B Produtor de Eventos</strong>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-5 space-y-4 shadow-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
                <div className="p-3 bg-slate-50 dark:bg-slate-950/60 rounded-lg border border-slate-200 dark:border-slate-800">
                  <span className="text-slate-500 dark:text-slate-400 block">Data de Criação</span>
                  <strong className="text-slate-900 dark:text-slate-200">{formatDate(proposal.createdAt)}</strong>
                </div>
                <div className="p-3 bg-slate-50 dark:bg-slate-950/60 rounded-lg border border-slate-200 dark:border-slate-800">
                  <span className="text-slate-500 dark:text-slate-400 block">Validade Comercial</span>
                  <strong className="text-slate-900 dark:text-slate-200">
                    {(currentVer?.validUntil || proposal.validUntil) ? formatDate(currentVer?.validUntil || proposal.validUntil) : 'Indeterminada'}
                  </strong>
                </div>
                <div className="p-3 bg-slate-50 dark:bg-slate-950/60 rounded-lg border border-slate-200 dark:border-slate-800">
                  <span className="text-slate-500 dark:text-slate-400 block">Vigência Proposta</span>
                  <strong className="text-slate-900 dark:text-slate-200">
            ...
            {/* Box: Observações */}
            {proposal.notes && (
              <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-5 space-y-2 shadow-xs">
                <h3 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider">
                  Notas Internas
                </h3>
                <p className="text-xs text-slate-600 dark:text-slate-400 whitespace-pre-line">{proposal.notes}</p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-6">
            {/* Produtor Card */}
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-5 space-y-3 shadow-xs">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Produtor Parceiro</h3>
                <Building2 className="h-4 w-4 text-orange-500" />
              </div>
              <div>
                <h4
                  className="font-bold text-slate-900 dark:text-white hover:text-orange-500 cursor-pointer transition-colors"
                  onClick={() => onSelectProducer && onSelectProducer(proposal.producerId)}
                >
                  {proposal.producerName || 'Produtor B2B'}
                </h4>
                {(proposal as any).producerDocument && (
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{(proposal as any).producerDocument}</p>
                )}
              </div>
              <Button
                variant="outline"
                size="sm"
                className="w-full text-xs border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300"
                onClick={() => onSelectProducer && onSelectProducer(proposal.producerId)}
              >
            ...
            {proposal.opportunityId && (
              <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-5 space-y-3 shadow-xs">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Oportunidade CRM</h3>
            ...
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-5 space-y-3 text-xs shadow-xs">
              <h3 className="font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Governança Canônica</h3>
              <div className="space-y-2 text-slate-600 dark:text-slate-300">
                <div className="flex justify-between">
                  <span className="text-slate-500 dark:text-slate-400">Versão Ativa:</span>
                  <span className="font-mono font-bold text-slate-900 dark:text-white">v{activeVerNum}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-500 dark:text-slate-400">Versões Totais:</span>
                  <span className="font-bold text-slate-900 dark:text-white">{proposal.versions?.length || 1}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-500 dark:text-slate-400">Imutabilidade:</span>
                  <span className="text-emerald-600 dark:text-emerald-400 font-semibold">Ativa (SHA-256)</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {activeTab === 'terms' && (
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">Taxas & Serviços Negociados</h3>
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 overflow-hidden shadow-xs">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm text-slate-700 dark:text-slate-300">
                <thead className="bg-slate-50/80 dark:bg-slate-800/40 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
                  <tr>
                    <th className="px-5 py-3 font-semibold">Serviço / Oferta</th>
                    <th className="px-5 py-3 font-semibold">Tipo de Cobrança</th>
                    <th className="px-5 py-3 font-semibold">Taxa / Valor</th>
                    <th className="px-5 py-3 font-semibold">Base de Cálculo</th>
                    <th className="px-5 py-3 font-semibold">Quem Paga</th>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
                  {terms.length === 0 ? (
                    ...
                    terms.map((term, idx) => (
                      <tr key={term.id || idx} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-colors">
                        <td className="px-5 py-3 font-semibold text-slate-900 dark:text-white">
                          {term.offeringName || term.name || 'Serviço'}
                        </td>
                        <td className="px-5 py-3 text-xs">
                          <span className="rounded-lg bg-slate-100 dark:bg-slate-800 px-2 py-0.5 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 font-medium">
                        ...
                        <td className="px-5 py-3 font-mono font-bold text-orange-600 dark:text-orange-400">
                        ...
                        <td className="px-5 py-3 text-xs text-slate-500 dark:text-slate-400 max-w-xs truncate" title={term.conditions || ''}>
                          {term.conditions || '-'}
                        </td>
                      </tr>
                    ))
      ...
      {activeTab === 'events' && (
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">Eventos Abrangidos no Escopo</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
          ...
          <div className="p-3.5 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-500/30 rounded-xl text-xs text-amber-800 dark:text-amber-300/90 flex items-start gap-2">
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 overflow-hidden shadow-xs">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm text-slate-700 dark:text-slate-300">
                <thead className="bg-slate-50/80 dark:bg-slate-800/40 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
                  <tr>
                    <th className="px-5 py-3 font-semibold">Nome do Evento</th>
                    <th className="px-5 py-3 font-semibold">Local / Praça</th>
                    <th className="px-5 py-3 font-semibold">Data Prevista</th>
                    <th className="px-5 py-3 font-semibold">Público Estimado</th>
                    <th className="px-5 py-3 font-semibold">VGV Projetado</th>
                    <th className="px-5 py-3 font-semibold">Observações</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
                  {events.length === 0 ? (
                    ...
                    events.map((ev, idx) => (
                      <tr key={ev.id || idx} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-colors">
                        <td className="px-5 py-3 font-semibold text-slate-900 dark:text-white">{ev.estimatedEventName || ev.eventName || 'Evento'}</td>
                        <td className="px-5 py-3 text-xs text-slate-600 dark:text-slate-300">{ev.estimatedVenue || 'A definir'}</td>
                        <td className="px-5 py-3 text-xs text-slate-600 dark:text-slate-300">
                        ...
                        <td className="px-5 py-3 text-xs font-mono font-bold text-orange-600 dark:text-orange-400">
      ...
      {activeTab === 'versions' && (
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">Histórico de Versões Imutáveis</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
              ...
              return (
                <div
                  ...
                  className={`rounded-2xl border p-5 transition-all ${
                    isSelected
                      ? 'border-orange-500 bg-orange-50/20 dark:bg-slate-900/90 shadow-xs'
                      : 'border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 hover:border-slate-300 dark:hover:border-slate-700 shadow-xs'
                  }`}
                >
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div className="space-y-2">
                      <div className="flex items-center gap-3">
                        ...
                        {isActive && (
                          <span className="rounded-lg bg-orange-50 text-orange-700 border border-orange-200 dark:bg-orange-500/10 dark:text-orange-400 dark:border-orange-500/20 px-2 py-0.5 text-xs font-semibold">
                            Versão Vigente
                          </span>
                        )}
                      ...
                      {ver.contentHash && (
                        <div className="flex items-center gap-2 pt-1 font-mono text-xs text-slate-500 dark:text-slate-400">
                          ...
                          <span className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 px-2 py-0.5 rounded text-slate-700 dark:text-slate-300 select-all">
                            {ver.contentHash}
                          </span>
                          <button
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        ...
                        className={isSelected ? 'bg-orange-500 hover:bg-orange-600 text-white' : 'border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300'}
                      >
      ...
      {activeTab === 'document' && (
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">Documento Formal da Proposta</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
          ...
          {(docChecksum || currentVer?.contentHash) && (
            <div className="p-3.5 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 text-xs font-mono text-slate-600 dark:text-slate-400 flex items-center justify-between">
              <span>Checksum de Integridade do Documento:</span>
              <strong className="text-orange-600 dark:text-orange-400 select-all">
                {docChecksum || currentVer?.contentHash}
              </strong>
            </div>
          )}
          ...
          {docContent ? (
            <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white p-8 text-slate-900 shadow-sm overflow-auto max-h-[800px]">
              <div dangerouslySetInnerHTML={{ __html: docContent }} />
            </div>
          ) : (
            <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-16 text-center text-slate-500 dark:text-slate-400 shadow-xs">
              <FileText className="h-12 w-12 mx-auto mb-3 text-slate-400 dark:text-slate-600" />
              <h4 className="text-base font-bold text-slate-900 dark:text-white">Nenhum documento gerado ainda</h4>
      ...
      {activeTab === 'approvals' && (
        <div className="space-y-6">
          <div>
            <h3 className="text-base font-bold text-slate-900 dark:text-white">Governança & Alçadas Comerciais</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 space-y-3 shadow-xs">
              <div className="flex items-center justify-between">
      ...
      {activeTab === 'deliveries' && (
        <div className="space-y-6">
          <div>
            <h3 className="text-base font-bold text-slate-900 dark:text-white">Rastreamento de Envios & Aceite Formal</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">
          ...
          {/* Aceite Registrado */}
          {proposal.status === 'ACCEPTED' && (
            <div className="p-5 rounded-2xl border border-emerald-200 dark:border-emerald-500/40 bg-emerald-50 dark:bg-emerald-950/20 space-y-3 shadow-xs">
              <div className="flex items-center gap-2 text-emerald-700 dark:text-emerald-400 font-bold text-sm">
                <CheckCircle2 className="h-5 w-5" />
                <span>Aceite Comercial Registrado</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs text-slate-700 dark:text-slate-300 pt-2 border-t border-emerald-200 dark:border-emerald-900/40">
                <div>
                  <span className="text-slate-500 dark:text-slate-400 block">Status:</span>
                  <strong className="text-emerald-700 dark:text-emerald-300">Aceito pelo Produtor</strong>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400 block">Versão Formalizada:</span>
                  <strong className="text-slate-900 dark:text-slate-200">v{activeVerNum}</strong>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400 block">Próxima Etapa:</span>
                  <strong className="text-orange-600 dark:text-orange-400">Elaboração Contratual (1.3.6)</strong>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-5 space-y-4 shadow-xs">
            ...
            <div className="space-y-3">
              <div className="p-3.5 bg-slate-50 dark:bg-slate-950/60 rounded-xl border border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs">
                <div className="flex items-center gap-3">
                  <Send className="h-4 w-4 text-cyan-600 dark:text-cyan-400" />
                  <div>
                    <strong className="text-slate-900 dark:text-slate-200">Envio de Formalização Comercial</strong>
                    <p className="text-slate-500 dark:text-slate-400">Destinatário principal do produtor</p>
                  </div>
                </div>
