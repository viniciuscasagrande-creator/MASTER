    ...
    <div className={cn('grid gap-4', colClassMap[columns], className)}>
      {metrics.map((m) => (
        <div
          ...
          onClick={m.onClick}
          className={cn(
            'group relative overflow-hidden rounded-2xl border border-slate-800/80 bg-slate-900/60 p-5 shadow-lg backdrop-blur-sm transition-all duration-200 hover:border-slate-700 hover:bg-slate-900/90',
        ...
        >
          <div className="flex items-center justify-between gap-2">
            ...
            {m.icon && (
              <div className="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-800 bg-slate-950/60 text-slate-300 group-hover:text-white transition-colors">
                {m.icon}
              </div>
            )}
          </div>
          ...
          <div className="mt-2.5 flex items-center justify-between text-xs pt-2 border-t border-slate-800/40">
            {m.helper && (
